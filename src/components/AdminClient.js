...
export default function AdminClient(){
 ...
 const load=useCallback(()=>fetch("/api/dishes?admin=1",{cache:"no-store"}).then(r=>r.json()).then(d=>{setItems(d);return d}),[]);
 ...
 const save=async()=>{await fetch("/api/dishes/"+draft.id,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(draft)});await load();toast("Saqlandi")};
 const add=async()=>{const r=await fetch("/api/dishes",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(empty)}),d=await r.json();await load();setSelectedId(d.id);setDraft(d);toast("Yangi taom qo‘shildi")};
 const remove=async()=>{if(!confirm("Taom o‘chirilsinmi?"))return;await fetch("/api/dishes/"+draft.id,{method:"DELETE"});const next=await load();setSelectedId(next[0]?.id??null);setDraft(next[0]?structuredClone(next[0]):null);toast("O‘chirildi")};
